...
テキスト読み上げは [Qwen3-TTS](https://github.com/QwenLM/Qwen3-TTS) のゼロショット音声クローン。
**本プロジェクトは Qwen / Alibaba とは無関係です。**

---

## 普通のテキスト読み上げ(TTS)と何が違うのか
...
```mermaid
%% **縦向き (TB)。**横向きだと図が右へ伸びて、GitHub の mermaid ビューアの
%% 操作ボタン (右端に固定) と重なり、右端のノードが隠れる。
flowchart TB
  N1["🎙️ 参照音声 ＋ 書き起こし<br/>(3〜10秒)"] --> NM["🧠 読み上げモデル<br/>Qwen3-TTS"]
  N2["📝 読み上げる文"] --> NM
  NM --> NO["🔊 読み上げ音声"]
```
...
%% 縦向きの理由は上の図のコメントを参照。
flowchart TB
  C1["🎙️ 参照音声 ＋ 書き起こし<br/>(3〜10秒)"] --> CX["☠️ 崩壊変換"]
  CX --> CB["🧟 壊れた参照音声"]
  CB --> CM["🧠 読み上げモデル<br/>Qwen3-TTS"]
  C2["📝 読み上げる文"] --> CM
  CM --> CO["💀 崩壊音声"]
...
| <b>プリヤ・シャルマ</b><br><video src="https://github.com/user-attachments/assets/3650505b-ac32-4931-96af-7e4c89f9101d"></video><br><sub>12.9s</sub> | <video src="https://github.com/user-attachments/assets/2ca554bf-b8b5-4648-9b46-a4ca75214676"></video><br><sub>seed 1</sub> | —🚧うまく壊せず🚧— |—🚧うまく壊せず🚧— |
...
本ツールは**ジョークツール**ですが、使い方次第で人を傷つけます。
**[TERMS.md](TERMS.md) を読んでから関係者全員が楽しめる範囲で使ってください。** 要点だけ:
